...
export const Board = ({ columns, setColumns, tasks, userId, doneColumnId }) => {
  ...
  return (
    <>
      <main className="flex-1 flex items-start overflow-x-auto p-4 space-x-4 bg-gray-900">
        ...
        <div className="flex-shrink-0 w-80">
            <button 
                ...
                className="w-full p-2 bg-white/10 hover:bg-white/20 rounded-lg flex items-center text-gray-400 transition-colors"
            >
      ...
      <Modal isOpen={isColumnModalOpen} onClose={() => setColumnModalOpen(false)} title="Add New Column">
        <form onSubmit={handleAddColumn}>
          <input type="text" value={newColumnName} onChange={(e) => setNewColumnName(e.target.value)} placeholder="Enter list title..." className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500" required />
